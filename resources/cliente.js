...
class Client{
        ...
        try{
            ...
            this.socket.on('connect', () => {
                ...
                this.socket.on('enc_method', (socket)=>{
                    this.enc_method = socket.enc_method;
                });
            });
        }catch(error){
            window.location.href = 'http://localhost/FamilyNet/main';
    ...
    receive_message(socket, str){
        var decrypted_msg = this.decrypt_message(str);

        $(".messages").append('<p class="message">'+decrypted_msg+'</p>');
    }

    encrypt_message(str){
        var new_str = '';
        for(let i = 0; i < str.length; i++){ // Percorre a string passada
            new_str += this.enc_met[str[i]]; // E para cada letra, substitui pela correspondente no método de encriptação gerado no servidor
        }
    ...
    decrypt_message(str){       
        ...
        for(var i = 0; i < str_cod.length; i++){ // Percorre a string baralhada
            var original_char = Object.keys(this.enc_met).find(key => this.enc_met[key] === str_cod[i]); // Substitui cada caractere pelo caractere correto, seguindo o método de encriptação criado no servidor
